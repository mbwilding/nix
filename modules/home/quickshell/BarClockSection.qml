...
Item {
    ...
    function timeText(d) {
        if (!d) return "--:--";
        if (Config.bar.clock24h) return Qt.formatTime(d, "HH:mm");
    ...
    HoverHandler {
        onHoveredChanged: {
            if (hovered) clockSection.openPopupReq("clock");
            else         clockSection.keepPopupReq();
        }
    }
    ...
    Rectangle {
        ...
        HoverHandler {
            onHoveredChanged: {
                if (hovered) clockSection.openPopupReq("clock");
                else         clockSection.exitPopupReq();
            }
        }
        ...
        RowLayout {
            ...
            Text {
                ...
                text: Qt.formatDate(
                    new Date(calendarPopup.displayYear, calendarPopup.displayMonth - 1, 1),
                    "MMMM yyyy"
                )
                color: Config.colors.textPrimary
                font.family:    Config.font.family
