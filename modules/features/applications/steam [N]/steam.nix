...

{
  ...
  flake.modules.homeManager.steam =
    { pkgs, lib, ... }:
    let
      protonVersion = pkgs.lib.trim (builtins.readFile "${pkgs.proton-ge-bin.steamcompattool}/version");
      protonName = builtins.elemAt (pkgs.lib.splitString " " protonVersion) 1;
    in
    let
      defaultTemplateFile = pkgs.writeText "steamtinkerlaunch-default-template.conf" ''
        ...
        ## redirect will only create new compatdata directories for major Proton version changes, such as Proton 8.0, and will ignore minor fix versions, such as Proton 8.0-3c -- Back up any existing compatdatas before using this option
        ONLYPROTMAJORREDIRECT="0"
        ...
        MAHUBIN="/run/current-system/sw/bin/mangohud"
        ## The arguments used for the MANGOHUD_CONFIG environment variable
        MAHUARGS="none"
        ## Append '--dlsym' to MangoHud - May fix OpenGL games not displaying MangoHud
        MAHUDLSYM="0"
        ...
        USEMANGOHUDSTLCFG="0"
        ## select a Vulkan Post-Processor for use with a Vulkan title (Native Linux or Proton compatible) - This option SHOULD NOT be used with ReShade enabled
        VULKANPOSTPROCESSOR="none"
        ...
        RUN_X64DBG="0"
        ## when enabled, the game will not launch automatically and will be attached to x64dbg -- When disabled, the game and x64dbg will start at the same time, and you will have to manually attach the process
        X64DBG_ATTACHONSTARTUP="1"
        ...
        RESHADE_DEPTH3D="0"
        ## Start game with ReShade enabled, downloading the default or overridden version before game launch
        USERESHADE="0"
        ...
        RESHADEUPDATE="0"
        ## Disable creating the ReShade INI file when using ReShade on first install or if missing, will do nothing if ReShade is disabled -- This will NOT remove an existing ReShade INI, it will only avoid (re)creating it if one is not found
        CREATERESHINI="1"
        ## name to give ReShade DLL (default=dxgi) - d3d9=Direct3D 9, d3d11.dll for DX11, dxgi=DX10/11/12, opengl32/64=OpenGL, or custom name. You can comma-separate DLL names for multiple ReShade DLLs. Existing DLLs with this name will get backed up in the game directory
        RESHADEDLLNAME="dxgi.dll"
        ...
        WAITEDITOR="2"
        ## Create steam_appid.txt file in gamedir if enabled or delete it if disabled -- This option has no effect for Non-Steam Games
        STEAMAPPIDFILE="0"
        ...
        DXVK_ASYNC="1"
        ## Enable DXVK HDR support, requires compatible GPU and drivers - This option is ignored on Steam Deck Game Mode, and is ignored if GameScope HDR is enabled
        DXVK_HDR="1"
        ...
        ## enable Per-Game Wine DPI value set below for this game only
        USEPERGAMEWINEDPI="0"
        ## set a Wine DPI value for this game only, this will override any global Wine DPI value - Default is '96' as set by Wine
        PERGAMEWINEDPI="96"
        ...
        WAITMO2="2"
        ## use a different executable configuration in Silent Mode (i.e. launch 'SKSE64' instead of 'Skyrim Special Edition') -- When 'none' by default, ModOrganizer 2 will launch the configuation matching the name of the INI (i.e. Skyrim Special Edition)
        MO2SILENTMODEDESC_MO2SILENTMODEEXEOVERRIDE=""
        ## Mod Organizer 2 mode
        MO2MODE="disabled"
        ## Enable Special K for the selected game
        USESPECIALK="0"
        ## Choose the name for the SpecialK DLL, default is 'auto' and chosen based on game renderer (if known) - Override to set specific DLL name for specific rendering backend or to avoid DLL name conflicts
        SPEKDLLNAME="auto"
        ## copy ReShade DLLs without renaming them and let SpecialK manage loading them - If disabled, ReShade will be loaded by the game instead of SpecialK and will IGNORE any Global SpecialK ReShade Override Version, which may be more desirable in some cases
        USERESHSPEKPLUGIN="1"
        ...
        ## Enables GameScope WSI
        USEGAMESCOPEWSI="1"
      '';
    in
    {
      home.activation.steamtinkerlaunchDefaultTemplate = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        target="$HOME/.config/steamtinkerlaunch/default_template.conf"
        if [ ! -e "$target" ]; then
          install -Dm644 ${defaultTemplateFile} "$target"
        fi
      '';
      ...
      home.activation.steamtinkerlaunchGamecfgs = lib.hm.dag.entryAfter [ "writeBoundary" ] (
        ...
        ''
          if [ -d "${gamecfgDir}" ]; then
            for f in "${gamecfgDir}"/*.conf; do
              [ -f "$f" ] || continue
              ${pkgs.gnused}/bin/sed -i \
                's|^USEPROTON=.*|USEPROTON="${protonName}"|' \
                "$f"
            done
          fi
        ''
